			...
			<el-tabs v-model="state.selectTabs" type="border-card" @tab-click="handleClick">
					...
					<el-form ref="formData" label-width="auto" :rules="state.rules" :model="state.formData"
						...
						<el-row :gutter="20">
							<el-col v-if="state.formData.id" :span="8">
								<el-form-item label="订单单号" prop="fapiaoNo">
				...
				<el-tab-pane name="fpInfo" label="订单信息">
					<el-descriptions title="" :border="true">
						<el-descriptions-item label="订单单号">{{ state.fpInfo?.code || '' }}</el-descriptions-item>
						<el-descriptions-item label="客户名称">{{ state.fpInfo?.titlename || '' }}</el-descriptions-item>
						<el-descriptions-item label="订单申请号">{{ state.fpInfo?.fapiaoNo || '' }}</el-descriptions-item>
						<el-descriptions-item label="订单金额">{{ state.fpInfo?.amount || '' }}</el-descriptions-item>
					</el-descriptions>
				</el-tab-pane>
			</el-tabs>
		</div>
